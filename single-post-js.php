   ...
   //I will get to see if this code is useful for this comment system
    ...
    $(document).on('click', '.post_reply', function(){
        ...
        $.ajax({
            url:"action.php",
            method:"POST",
            data:{comment_id:comment_id, email:email, action:action},
            success:function(data){
      ...
      event.preventDefault();
        ...
        if(replies != '')
        {
            $.ajax({
                url:"action.php",
                method:"POST",
                data:{comment_id:comment_id,email:receiver_id,replies:replies,action:action},
                success:function(data)
                {  
                     fetch_post();  
  ...
  if(edit_reply == '')
        ...
        {             
            $.ajax({
                ...
                data: {editReply_id:id, edit_reply:edit_reply},
                success: function(msg)
                {  
                    if(msg == 'ok'){
                      //confirmation of update               
...
//report section start
  ...
  $(document).on('click', '.reportPostID', function(event){
    event.preventDefault();
      ...
      var logged = '<?php if (!empty($_SESSION['id'])) { echo $_SESSION['id']; } ?>';
...
$(document).on('click', '.reportReplyID', function(event){
  ...
  alert('Choose Report Type');
      ...
      $.ajax({
            type:'POST',
            url:'reportReply.php',
            data:{report:report_checked, reply_postID:reply_postID, page_post:pageId, reported_email:reported_email, reported_content:reported_content, reported_user:reported_user},           
            success:function(response){
                if(response == 'ok'){     
                         $('.responseMsg').html('<span style="color:green; text-align:center;">Report Submitted!.. Under Review!</p>');
                          setTimeout(
    ...
    event.preventDefault();
       ...
       var type = 0;
        ...
        $.ajax({
            ...
            type: 'post',
            data: {reply_id:reply_id,type:type},
            dataType: 'json',
            success: function(data){
...
$(document).on('click', '#oldest', function(event){
 ...
 function fetch_oldest()
    ...
    // sql query for top comments
//https://stackoverflow.com/questions/23500057/counting-likes-and-comments-between-two-different-table-of-a-specific-topic
